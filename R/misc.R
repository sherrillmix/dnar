...
insetScale<-function(breaks,col,insetPos=c(.025,.015,.04,.25),label=''){
  ...
  graphics::rect(breakPos[-1]-1e-4*diff(range(breakPos)),insetPos[1],breakPos[-length(breakPos)],insetPos[3],col=col[-1],xpd=NA,border=NA)
  ...
  prettyPos<-(prettyLabs-(min(breaks)))/((max(breaks))-(min(breaks)))*(insetPos[4]-insetPos[2])+insetPos[2]
  graphics::segments(prettyPos,insetPos[1],prettyPos,insetPos[1]-diff(insetPos[c(1,3)])*.1,xpd=NA)
  graphics::text(prettyPos,insetPos[1]-diff(insetPos[c(1,3)])*.175,prettyLabs,xpd=NA,adj=c(.5,1),cex=.85)
  graphics::text(mean(insetPos[c(2,4)]),insetPos[3]+diff(insetPos[c(1,3)])*.45,label,xpd=NA,adj=c(.5,0))
  invisible(NULL)
}
