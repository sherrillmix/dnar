...
logAxis<-function(side=2,exponent=TRUE,addExtra=!exponent,minorTcl=-.2,axisMin=-Inf,offset=0,...){
  ...
  graphics::axis(side,allTicks+offset,rep('',length(allTicks)),tcl=minorTcl)
  if(ceiling(log10(minX))<=floor(log10(maxX)))prettyY<-seq(ceiling(log10(minX)),floor(log10(maxX)),1)
  else prettyY<-c()
  graphics::axis(side,10^prettyY+offset,rep('',length(prettyY)),tcl=minorTcl*2)
  ...
  graphics::axis(side,10^prettyY+offset,labs,...)
...
#' @param insetPos a four element numeric vector giving the position of the left, top, right and bottom edges of the scale in nfc coordinates (0 = bottom/left of figure, 1 = top/right of figure)
#' @param label a character string giving label for color scale
#' @export
...
#' insetScale(breaks,cols,label='Distance')
insetScale<-function(breaks,col,insetPos=c(.015,.025,.25,.04),label=''){
  if(length(breaks)!=length(col)+1)stop('Number of breaks must be one more than colors')
  insetPos<-c(graphics::grconvertX(insetPos[1],'nfc','user'),graphics::grconvertY(insetPos[2],'nfc','user'),graphics::grconvertX(insetPos[3],'nfc','user'),graphics::grconvertY(insetPos[4],'nfc','user'))
  ...
  graphics::text(prettyPos,insetPos[2]-diff(insetPos[c(2,4)])*.175,prettyLabs,xpd=NA,adj=c(.5,1),cex=.85)
  graphics::text(mean(insetPos[c(1,3)]),insetPos[4]+diff(insetPos[c(2,4)])*.45,label,xpd=NA,adj=c(.5,0))
  invisible(NULL)
}
