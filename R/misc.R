...
#' @param axisMax The maximum to extend ticks to
...
#' @param axisVals log10 values to be displayed on y axis (instead of autogenerated)
...
#' @param labYOffset a vector of the same length as labels to be used to fine tune label positioning. Values given proportional to plot size
#' @param labXOffset a vector of the same length as labels to be used to fine tune label positioning. Values given proportional to plot size
#' @param cex character size for axis and labels
#' @export
...
#' @param axisArgs a list of additional arguments for axis
#' @param align either 'c' for centered text or 'l' for left justified text
#' @param ... additional arguments to text
#' @export
...
multiYAxis<-function(pos,labs,spacer=.3,start=1,cex=1.1,axisArgs=list(),align=c('l','c'),...){
  ...
  centers<-c(rev(cumsum(rev(widths)+spacer))[-1],0)+widths*ifelse(align=='c',.5,0)
  graphics::text(rep(start-centers,each=nrow(labs)),rep(pos,ncol(labs)),unlist(labs),xpd=NA,cex=cex,adj=ifelse(align=='c',.5,1),...)
